...
| `center` |         | Pinned to the vertical centre of the screen               | Below the input      |
| `bottom` |         | Just above the bottom edge margin                         | **Above** the input  |
| `window` |   yes   | Card centred on the screen as a whole, input at its top   | Below the input      |

`window` is the default (the original layout): the whole card is vertically centred, so it grows symmetrically and the input shifts as the list changes. `center` instead keeps the input itself fixed at the screen's centre and grows the card downward. With `bottom`, the lower area stacks above the input and grows upward.
...
The values are re-read on each summon, so an edit takes effect the next time you open the overlay without a shell restart. Any `position` other than the four above falls back to `window`, and a non-numeric or negative `edgeMargin` falls back to `5`.
