...
- **Full qalc reach** — arithmetic, powers, functions, unit conversion, and currency (`2+2`, `sqrt(625)`, `29 inches to cm`, `10 usd to gbp`). Offline; qalc uses its cached exchange rates.
...
| `Ctrl + 1…9` / `Ctrl + 0` | Copy history row 1–10, close (empty input) |
...
A `SUPER + =` hotkey can toggle it with:

```
omarchy-shell shell toggle icyleaf.qalculator '{}'
```

Note that `SUPER + =` is delivered as `code:21` (the `=` key without Shift), and Omarchy's default bind for that key is the tiling action "Shrink window left", so unbind `SUPER + code:21` first. The Shift/Alt/Ctrl resize variants can be left intact.
...
- `Overlay.qml` — the overlay: input, live answer, history list, qalc process, clipboard process.
...
- History is stored at `~/.local/state/omarchy/qalculator-history.json`, written atomically on commit only.
