...
- **输入框位置可配置** — 输入框可固定在 `top`、`center`（默认）或 `bottom`，也可用 `window` 让整张卡片整体居中。设为 `bottom` 时，历史与帮助列表显示在输入框上方；其余位置显示在下方。可在插件设置面板中设置，或直接写入 `shell.json`。
...
| :------- | :--: | :-------------------------------- | :------------------ |
| `top`    |      | 紧贴顶部间隙，卡片向下生长        | 输入框下方          |
| `center` |  是  | 钉在屏幕垂直正中                  | 输入框下方          |
| `bottom` |      | 紧贴底部间隙                      | 输入框**上方**      |
